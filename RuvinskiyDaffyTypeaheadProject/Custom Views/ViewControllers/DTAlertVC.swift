...
class DTAlertVC: UIViewController {
    
    let containerView = DTAlertContainerView()
    let titleLabel = DTTitleLabel(textAlignment: .center, fontSize: 20)
    let messageLabel = DTBodyLabel(textAlignment: .center)
    let actionButton = DTButton(backgroundColor: .systemPink, title: "Ok")
